...
impl Column {
    ...
    fn handle_foreign_rule(
        ...
        let rule_ref: &mut ForeignRule = rule_ref.as_mut().ok_or_else(|| {
            Error::new_spanned(attr, "This field does not have a foreign reference")
        })?;
...
impl ToTokens for Column {
    fn to_tokens(&self, tokens: &mut quote::__private::TokenStream) {
        ...
        tokens.extend(quote!(
          mensula::Column::new(
            #name,
            <#field_type as mensula::AsDataType>::as_data_type(),
            #modifier,
          )
        ))
    }
