<script lang="ts">
    ...
    import {
        addEditablePayment,
        editablePaymentArray,
    } from "$lib/db/editablePayment";
        ...
        if (errorCount === 0 && payments.length >= 1) {
            ...
            for (const payment of payments) {
                postPayments.push({
                    ...
                    ref_hash: payment.importInfo?.refHash,
                });
            }
    ...
    $: payments = updateEditablePayments($editablePaymentArray);

    $: tinkInfo = $tinkImportInfo;
...
<div class="col main">
    {#if tinkInfo !== undefined}
        <div class="tink-info col">
            <div class="row"><b>{tinkInfo.new}</b> Zahlungen importiert</div>
            <div class="row">
                <b>{tinkInfo.pending}</b> Zahlungen sind noch ausstehend
            </div>
            <div class="row">
                <b>{tinkInfo.listed}</b> Zahlungen wurden schon importiert
            </div>
        </div>
    {/if}
    {#if payments.length === 0}
        <div class="col center">
            <h2>Bitte neue Zahlung erstellen oder importieren</h2>
        </div>
    {:else}
...
<div class="col side">
    <div class="overview col">
        ...
        <button
            class="row center"
            on:click={submitPayments}
            disabled={errorCount !== 0}>Abschicken <Icon icon="send" /></button
        >
    </div>
